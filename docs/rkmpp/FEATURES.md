...
- Vulkan UI 使用固定版本的 Dear ImGui core 与官方 Vulkan renderer backend 离屏绘制 960x180 不透明诊断页，再由 RGA 同步覆盖到 HDMI RX DMA-BUF；不创建 GLFW/SDL 窗口或 swapchain。相同 revision 复用缓存。
