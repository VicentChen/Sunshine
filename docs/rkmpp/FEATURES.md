...
- Vulkan UI 使用固定版本的 Dear ImGui core 与官方 Vulkan renderer backend 离屏绘制不透明 BGR 诊断页；页面尺寸、字体和留白按 Moonlight 输出分辨率及用户选择的紧凑/标准/大号档位缩放。单会话、尺寸匹配的 BGR888 capture 由 Vulkan 直接覆盖其 DMA-BUF ROI，不经过 RGA；通过严格布局校验的独占 NV12 capture 则等待 Vulkan 发布面板后，由同步 RGA 将 BGR 面板转换为 BT.709 limited 并写入原始 capture ROI，再把同一帧交给 MPP。NV12 必须为逐行、单平面、偶数尺寸、64-byte 对齐 stride、零 data_offset，且 sizeimage、bytesused、payload 与 stride × height × 3/2 完全一致；仅 allocation_size 可以含页对齐余量。每次写入结束立即释放 RGA destination import，capture lease 持有至编码消费结束。尺寸不匹配、多会话共享输入或不满足这些条件时仍使用会话私有 CMA BGR888 target。UI 隐藏且尺寸/layout 匹配时保持零拷贝直通。不创建 GLFW/SDL 窗口或 swapchain；相同 revision 复用缓存。UI 仅持有当前页面族的一套 CMA surface；普通页/Profile 切换或尺寸变化时先销毁旧 surface 和 renderer import，再建立新 surface，避免两套页面常驻以及新旧分配叠加的峰值。
- NV12 原位 UI 写入失败时不编码该帧：关闭 UI 并让现有 worker 错误处理结束会话，避免继续使用可能部分修改的 capture 数据。4K NV12 原位路径省去两个完整 BGR888 私有 target，减少约 47.46 MiB CMA 分配；UI 面板本身仍需 DMA-BUF。Profile 的 `ui_compose` 包含局部 RGA 转换，视频缩放的 bypass 状态不能理解为没有任何 UI RGA 工作。
- 2026-09-05 独立实机探针验证了 Vulkan 发布 → 同步 RGA ROI → 同一 capture fd 的 MPP HEVC 编码：4K NV12、2880×1440 面板连续 1,200 帧完成，抽检 ROI 与独立转换参考一致、ROI 外字节未变，整段码流解码为 1,200 帧且首尾图像正常；底部居中 2560×1440 面板另有 120 帧逐帧检查通过。这些测试未复现历史绿屏，不能将其归因为 capture allocation 天生不可写。新接入的 Sunshine 原位分支已构建并通过 61 项针对性单测；每帧释放 RGA destination import 的追加长测因 HDMI 无信号在首帧前退出，不计入通过。随后用户实测反馈运行正常并确认提交；该反馈不代表未执行的长测或完整故障恢复矩阵已经通过。
- 可见 UI 的预处理缓冲区发生 `ENOMEM` 时，释放本会话 UI surface/import 并禁用叠加，对保留的同一原始帧仅重试一次无 UI 路径：尺寸匹配时直通，否则使用 NV12 RGA target。此保护不扩大系统 CMA，也不保证内存不足时仍可显示 UI；基础视频路径失败或其他异常仍保留具体错误。预处理启动失败、worker 异常终止或编码失败会结束会话，避免紧密重建编码器直到 Moonlight 报收不到视频。
...
- UI 根窗口的不透明背景由 Vulkan render-pass clear 直接生成，不再由 ImGui 重复绘制覆盖整个面板的背景矩形；卡片、文字、滚动与页面布局保持原有绘制。Timeline 最新帧与平均视图各只分配每阶段一个条目的临时空间，仍统计全部 32 个历史帧，并在访问累计数组前拒绝非法阶段编号。
- Vulkan UI 的 BGR surface 使用适合 Vulkan external-buffer import 的对齐行跨度；Compact（85%）Profile 不再因未对齐的 1224 像素可见宽度创建失败。尺寸或页面族切换不重叠持有新旧 surface；运行时创建失败时重建最后可用布局，重建也失败才禁用 UI。
...
- UI controller 的输入截获由成功初始化的 renderer backend 引用计数门控；最后一个后端退出或运行失败时会立即关闭模态并清理按键状态。HDMI RX capture generation 在 UI 可见性判断前更新，source recovery 会主动销毁当前页面 renderer 的旧 BGR DMA-BUF import，即使 UI 当时隐藏也不会复用旧 allocation。
