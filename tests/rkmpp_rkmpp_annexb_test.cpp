...
int main() {
  const auto expect = [](bool condition, const char *name) {
    if (!condition) {
      std::cerr << "rkmpp_rkmpp_annexb_test=FAIL: " << name << '\n';
    }
    return condition;
  };
  ...
  passed = expect(!platf::rkmpp::detail::output_is_idr(false, h264_idr.data(), h264_idr.size(), h264), "Non-intra packet skips IDR confirmation") && passed;
  passed = expect(platf::rkmpp::detail::output_is_idr(true, h264_idr.data(), h264_idr.size(), h264), "Intra H264 IDR confirmation") && passed;
  if (!passed) {
    return 1;
  }
