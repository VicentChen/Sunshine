...
namespace {
  ...
  std::size_t fd_count() {
    ...
    while (const auto *entry = readdir(directory)) {
      if (entry->d_name[0] != '.' && std::string(entry->d_name) != std::to_string(directory_fd)) {
        ++count;
      }
    }
    closedir(directory);
  ...
  options_t parse(int argc, char **argv) {
    ...
    for (int index = 1; index < argc; ++index) {
      ...
      if (argument == "--codec" && index + 1 < argc) {
        ...
        if (value == "h264") {
          options.codec = platf::rkmpp::codec_e::h264;
        } else if (value == "h265") {
          options.codec = platf::rkmpp::codec_e::h265;
        } else {
        ...
        if (argument == "--frames") {
          options.frames = static_cast<std::uint32_t>(value);
        } else {
          options.sessions = static_cast<std::uint32_t>(value);
        }
    ...
    if (!codec_set) {
      throw std::invalid_argument("--codec is required");
    }
    if (options.output.empty()) {
      options.output = options.codec == platf::rkmpp::codec_e::h264 ? "/tmp/rkmpp.h264" : "/tmp/rkmpp.h265";
    }
    return options;
  }
  ...
  bool annexb_has_required_nals(const std::string &path, platf::rkmpp::codec_e codec) {
    ...
    for (std::size_t index = 0; index + 4 < bytes.size(); ++index) {
      ...
      if (bytes[payload] == 0 && bytes[payload + 1] == 0 && bytes[payload + 2] == 1) {
        payload += 3;
      } else if (bytes[payload] == 0 && bytes[payload + 1] == 0 && bytes[payload + 2] == 0 && bytes[payload + 3] == 1) {
        payload += 4;
      } else {
        continue;
      }
  ...
  std::vector<std::vector<unsigned char>> annexb_nals(const std::string &path) {
    ...
    for (std::size_t index = 0; index < starts.size(); ++index) {
      ...
      if (start < end) {
        nals.emplace_back(bytes.begin() + start, bytes.begin() + end);
      }
    }
    return nals;
  }
  ...
  bool h264_first_mb_is_zero(const std::vector<unsigned char> &nal) {
    ...
    for (std::size_t index = 1; index < nal.size(); ++index) {
      if (index + 2 < nal.size() && nal[index] == 0 && nal[index + 1] == 0 && nal[index + 2] == 3) {
        ...
        rbsp.push_back(nal[index]);
      }
    }
    std::size_t bit = 0, leading_zeroes = 0;
    while (bit < rbsp.size() * 8 && !(rbsp[bit / 8] & (0x80 >> (bit % 8)))) {
      ++leading_zeroes;
      ++bit;
    }
    if (bit >= rbsp.size() * 8) {
  ...
  std::size_t access_unit_count(const std::string &path, platf::rkmpp::codec_e codec) {
    ...
    for (const auto &nal : annexb_nals(path)) {
      if (codec == platf::rkmpp::codec_e::h264) {
        const auto type = nal.front() & 0x1f;
        if ((type == 1 || type == 5) && h264_first_mb_is_zero(nal)) {
          ++count;
        }
  ...
  std::size_t run_session(const options_t &options, std::uint32_t session, std::string output_path) {
    {
      ...
      if (!input_layout.has_value()) {
        throw std::runtime_error("HDMI RX format has no valid RKMPP direct layout");
      }
      ...
      for (std::uint32_t frame_index = 0; frame_index < options.frames; ++frame_index) {
        ...
        platf::rkmpp::input_frame_t input {
          *input_layout,
          ...
          holder,
          nullptr,
          platf::rkmpp::input_buffer_key_t {holder->generation(), holder->buffer_index()},
        };
        encoder.encode(input, output);
        input.holder.reset();
        holder.reset();
      }
      if (fd_count() != live_baseline) {
        throw std::runtime_error("fd count changed while encoding");
      }
      output.close();
      const auto encoder_stats = encoder.stats();
      if (encoder_stats.frames != options.frames) {
        throw std::runtime_error("encoded frame count mismatch");
      }
      if (!annexb_has_required_nals(output_path, options.codec)) {
        throw std::runtime_error("stream lacks required parameter sets or IDR");
      }
      if (!first_key_unit_has_required_nals(output_path, options.codec)) {
        throw std::runtime_error("first key unit lacks required parameter sets");
      }
      const auto stream_stats = bitstream_stats(output_path, options.codec);
      if (stream_stats.access_units != options.frames) {
        throw std::runtime_error("bitstream access-unit count mismatch");
      }
...
int main(int argc, char **argv) {
  try {
    ...
    if (warmup_fd != steady_process_fd) {
      throw std::runtime_error("fd count changed after warmup teardown");
    }
    ...
    for (std::uint32_t session = 1; session <= options.sessions; ++session) {
      ...
      if (session > 1 && after_session_fds != previous_session_fds) {
        throw std::runtime_error("fd count grew across complete encoder sessions");
      }
      previous_session_fds = after_session_fds;
    }
    const auto final_process_fd = fd_count();
    if (final_process_fd != steady_process_fd) {
      throw std::runtime_error("fd count changed after all session teardown");
    }
