...
osd_region_status_e validate_osd_region(const osd_region_t &region, std::uint32_t coded_width, std::uint32_t coded_height) noexcept {
  if (!region.width || !region.height || region.pixels.empty()) return osd_region_status_e::empty;
  if ((region.x | region.y | region.width | region.height) & 15U) return osd_region_status_e::unaligned;
  ...
  if (expected_size != region.pixels.size()) return osd_region_status_e::size_mismatch;
...
namespace {
  ...
  constexpr glyph_t glyph(char value) noexcept {
    ...
    if (value >= '0' && value <= '9') return digits[static_cast<std::size_t>(value - '0')];
    if (value >= 'A' && value <= 'Z') return letters[static_cast<std::size_t>(value - 'A')];
    switch (value) {
  ...
  void draw_text(std::array<std::uint8_t, frame_profile_overlay_bitmap_t::width * frame_profile_overlay_bitmap_t::height> &pixels,
                 std::uint32_t row, const char *text, std::uint8_t color) noexcept {
    ...
    for (std::uint32_t index = 0; text[index] && (index + 1U) * cell_width <= frame_profile_overlay_bitmap_t::width; ++index) {
      ...
      for (std::uint32_t column = 0; column < symbol.size(); ++column) {
        for (std::uint32_t bit = 0; bit < 7; ++bit) {
          if (!(symbol[column] & (1U << bit))) continue;
  ...
  void format_metric(char *buffer, std::size_t size, const char *label, const video::frame_profile_metric_snapshot_t &metric) noexcept {
    ...
    std::snprintf(buffer, size, "%-11s P50 %5.1f P95 %5.1f P99 %5.1f MS", label,
                  metric.p50_us / 1000.0, metric.p95_us / 1000.0, metric.p99_us / 1000.0);
  }
}  // namespace

frame_profile_overlay_bitmap_t::frame_profile_overlay_bitmap_t() noexcept { render_waiting(); }
...
std::span<const std::uint8_t> frame_profile_overlay_bitmap_t::pixels() const noexcept { return pixels_; }
...
struct encoder_t::state_t {
  ...
  ~state_t() {
    if (osd_buffer) mpp_buffer_put(osd_buffer);
...
encoded_packet_t encoder_t::encode_packet(const input_frame_t &input) {
  ...
  try {
    ...
    if (state_->osd_data.num_region) check(mpp_meta_set_ptr(frame_meta, KEY_OSD_DATA, &state_->osd_data), "mpp_meta_set_ptr(KEY_OSD_DATA)");
    if (!state_->stats.frames) check(state_->api->control(state_->context, MPP_ENC_SET_IDR_FRAME, nullptr), "MPP_ENC_SET_IDR_FRAME");
    if (input.profile) input.profile->mpp_submit_begin = std::chrono::steady_clock::now();
    check(state_->api->encode_put_frame(state_->context, input_frame.value), "encode_put_frame");
    if (input.profile) input.profile->mpp_submit_end = std::chrono::steady_clock::now();
    ...
    for (;;) {
      ...
      if (input.profile) input.profile->mpp_output = std::chrono::steady_clock::now();
      RK_S32 intra = 0;
...
void encoder_t::set_osd_region(const osd_region_t &region) {
  ...
  if (!state_->osd_buffer || mpp_buffer_get_size(state_->osd_buffer) < region.pixels.size()) {
    ...
    if (state_->osd_buffer) mpp_buffer_put(state_->osd_buffer);
...
void encoder_t::clear_osd() noexcept {
  if (state_) state_->osd_data.num_region = 0;
}
